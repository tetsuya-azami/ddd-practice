...
sealed interface ShopCode {
    companion object {
        ...
        fun new(value: String?): ShopCode {
            if (value.isNullOrBlank()) throw IllegalArgumentException("店舗コードを入力してください。")
            if (!value.matches(Regex(FORMAT))) throw IllegalArgumentException("店舗コードは${FORMAT}の形式で入力してください。")
